...
$sql = "SELECT * FROM `".$version."` AS tk INNER JOIN `book_info` AS bk ON bk.order = tk.b WHERE `title_short` LIKE '%$book%' AND `c` =$chapter AND `v` = $verse ";
$titleSql = "SELECT `title_short` FROM `".$version."` AS tk INNER JOIN `book_info` AS bk ON bk.order = tk.b WHERE `title_short` LIKE '%$book%' AND `c` =$chapter AND `v` = $verse ";
...
echo "<h1> " .  $firstTitle . " Chapter ". $chapter. " Verse ".$verse. " (".$bibleVersion .")</h1><br /> <hr />";
